// Player Points + Market Value system (in-game coin value).
//
//   Points      are derived automatically from a player's statistics ONLY.
//   Market Value is derived ONLY from the player's TOTAL POINTS.
//
// Losses, Decider Wins and Tournament Wins award 0 points and are intentionally
...
// Exact point rates (do NOT change unless explicitly instructed).
...
// Exact, non-overlapping, gap-free ranges (do NOT change).
// Format: [lowerBound, upperBound (null = unbounded), marketValueM]
...
// Return the Market Value in M coins for a given number of TOTAL POINTS.
...
// Format an M-coins Market Value for display, e.g. 120 -> "120M".
export function marketValueLabel(marketValueM: number): string {
  return `${marketValueM}M`;
}

// Convenience helper: compute points then the Market Value from those points.
